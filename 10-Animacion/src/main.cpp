...
void applicationLoop() {
	...
	while (psi) {
		...
		// Esto es para la luces pointlights //nomero de luces a utilizar de tipo pointlights =3
		shaderMulLighting.setInt("pointLightCount", 5);
		//posicion de la luz con indice [0]                                                      valores iguales que el posicionamiento de la primitiva
		...
		shaderMulLighting.setVectorFloat3("pointLights[4].position", glm::value_ptr((glm::vec3(-14.9, 4.8, -3.0))));
		...
		//Es para colocar las esferas de las luces
		...
		sphereLamp.setScale(glm::vec3(0.1, 0.1, 0.2));
		sphereLamp.setPosition(glm::vec3(-14.9, 4.8, -3.0));
		sphereLamp.setColor(glm::vec4(0.5, 0, 0.5, 1.0));
		sphereLamp.render();
		...
		///////P10 otra forma de escalar, posicionar y orientar 
		//Se contruye la casa
		glBindTexture(GL_TEXTURE_2D, textureID4);
		...
		case 1:
			...
			offsetAirfraftRot += 0.05;
			if (offsetAirfraftRot > 90.0) {
				offsetAirfraftRot =0.0;
				state =0;
			}
			break;
